...
export async function urlShortenerHandler(req, res, next) {
  ...
  try {
    ...
    if (attempts === 3) {
      logger.error("Failed to generate a unique short code after multiple attempts");
      return res.status(500).json({
        message: "Internal Server Error: Failed to generate unique short code. Please try again.",
      });
    }    
